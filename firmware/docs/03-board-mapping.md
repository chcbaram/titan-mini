...
| 기능 | 네트 | 핀 | 연결처 |
|---|---|---|---|
| UART1 TX | `TXD1` | **P707** | RPi 40핀 헤더 U18 (GPIO14 위치). **HSLink 디버거의 시리얼이 여기 연결돼 있다** |
| UART1 RX | `RXD1` | **P706** | 〃 (GPIO15 위치) |
| UART2 TX | `TXD2` | P801 | H1 3핀 헤더 — **DNP(미실장)** |
| UART2 RX | `RXD2` | P802 | 〃 |
...
UART1 이 HSLink 의 가상 시리얼 포트에 붙어 있어서, 디버거 케이블 하나로 적재와 콘솔이 동시에 된다.
