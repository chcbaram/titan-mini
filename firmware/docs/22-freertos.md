...
### RGB 는 한 패키지다 — 한 번에 한 색만

처음에는 채널을 용도별로 나눴다. RED 하트비트, GREEN 네트워크 링크, BLUE 활동.
**독립된 LED 세 개를 가정한 설계라 틀렸다.** 보드의 LED3 는 RGB 가 한 패키지에 든
것이라 링크(초록)가 켜진 상태에서 하트비트(빨강)가 켜지면 노란색이 된다.

정보는 **색**으로, 생존 여부는 **점멸**로 나르는 쪽이 맞다.

| 상태 | 색 | 패턴 | 의미 |
|---|---|---|---|
| BOOT | 파랑 | 켜짐 | 초기화 중 |
| RUN, 링크 없음 | 빨강 | 500 ms 점멸 | 정상. 네트워크 없음 |
| RUN, 링크 있음 | 초록 | 500 ms 점멸 | 정상. 네트워크 연결됨 |
| BUSY | 파랑 | 점멸 | 오래 걸리는 작업 중 |
| ERROR | 빨강 | 켜짐 | 오류 |

`ERROR` 와 `BOOT` 에서는 깜빡이지 않고 켜 둔다. 깜빡이면 정상 동작과 구분이 안 된다.

렌더링은 **요청한 색 외에는 반드시 끈다.** 그래야 색을 추가해도 섞이지 않는다.
...
void indicatorRender(Color_t color, bool on)
{
  ...
  {
    if (c == color && on) ledOn(color_led_ch[c]);
    else                  ledOff(color_led_ch[c]);
  }
}
```

검증은 포트 레지스터로 했다. `PODR` 상위 바이트의 bit8/9/10 이 G/R/B 이고 active low 다.

```
RUN + 링크 없음   0x0500 <-> 0x0700     빨강만 점멸
링크 up           0x0600 <-> 0x0700     초록만 점멸 (빨강 꺼짐)
ERROR             0x0500 고정           빨강 켜짐
```

> 직렬 저항이 색마다 달라(R 2 KΩ / G 12 KΩ / B 10 KΩ) 빨강이 가장 밝다.
